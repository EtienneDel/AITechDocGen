...
const generateDocsForFunctionBatch = async (functionsByFile, apiKey) => {
    const prompt = (0, prompt_1.createPromptForFunctions)(functionsByFile.flatMap(({ functions }) => functions));
    ...
    if (!response.ok) {
        throw new Error(`Claude API error: ${response.status.toString()} ${response.statusText}`);
    }
